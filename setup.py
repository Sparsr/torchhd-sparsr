...
def _stage_native(root: pathlib.Path) -> None:
    """Copy the five libraries into the package so the wheel carries them."""
    if NATIVE_DIR.exists():
        shutil.rmtree(NATIVE_DIR)
    NATIVE_DIR.mkdir(parents=True)
    for lib_name in NATIVE_LIBS:
        shutil.copy2(root / "lib" / lib_name, NATIVE_DIR / lib_name)
...
def _torch_pin() -> str:
    """An exact pin on the PyTorch that compiled this wheel.

    `_C` uses libtorch's C++ ABI directly, and PyTorch does not keep that ABI
    stable between minor versions, so a wheel works against the torch that built
    it and no other. A range would let pip install it against any torch from 2.0
    onwards and crash on import.
    ...
    2.14 just as the reverse does. It only moves the single supported version
    ...
    versions cannot coexist under one package version. Supporting another torch
    means another release of this package, which is exactly what torchvision
    does: 0.29.0 pins torch==2.14.0, 0.28.0 pins torch==2.13.0.

    The local version is stripped so that `torch==2.14.0` also accepts the CUDA
    builds, `2.14.0+cu121` and the rest.

    Derived from the artifact rather than written by hand, the same rule the
    manylinux tag follows: hard-coding it would let a change to the build
    quietly promise compatibility the binary no longer has.
    """
...
setup(
    install_requires=[_torch_pin(), "torch-hd>=5.0", "numpy"],
